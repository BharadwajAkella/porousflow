...
void PorousFlowScene::updateFluidObjects(){	
	...
	for (int i = 0; i < fluidParticles.size(); i++){
		fluidParticles[i].step1();
	}
	for (int i = 0; i < fluidParticles.size(); i++){
		if (!fluidParticles[i].onboundary){
			if (fluidParticles[i].cfGradLength > stThreshold)
				fluidParticles[i].stForce = fluidParticles[i].surfaceTension * fluidParticles[i].cfLaplacian * fluidParticles[i].cfGradDirection;
			else
				fluidParticles[i].stForce = vec3(0.f);
			vec3 stForce = fluidParticles[i].stForce;
			vec3 vForce = fluidParticles[i].viscousForce;
			vec3 pForce = fluidParticles[i].pressureForce;

			vec3 force = stForce + pForce + vForce + fluidParticles[i].bodyForce;
			vec3 acc = force / fluidParticles[i].density;
			if (acc.x > 1000 || acc.y > 1000 || acc.z > 1000){
				cout << "acc blasted" << endl;
			}

			fluidParticles[i].velocity += deltaTime * acc;
